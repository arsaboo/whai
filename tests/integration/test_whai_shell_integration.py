...
def whai_shell_session(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "whai.context.session_reader.get_config_dir", mock_get_config_dir
    )
    monkeypatch.setattr(
        "whai.configuration.user_config.get_config_dir", mock_get_config_dir
    )
...
def _run_commands_in_recorded_shell(
    commands: list[str], cwd: Path, log_path: Path, is_windows: bool
):
    ...
    if is_windows:
        ...
        if variant == "util-linux":
            # util-linux: script -qf -c "commands" log
            cmd = [script_bin, "-qf", "-c", cmd_sequence, str(log_path)]
        elif variant == "bsd":
...
def test_whai_shell_full_session_integration(whai_shell_session, monkeypatch):
    ...
    config = create_test_config(
        default_provider="openai", default_model="gpt-4", api_key="test-key"
    )
...
The end."""
    ...
    if is_windows:
        ...
        assert "Once upon a time" in whai_content, (
            f"Whai log should contain story: {whai_content[:200]}"
        )
    ...
    if is_windows:
        assert "Once upon a time" in context_with_command, (
            "Story response should be in context"
        )
    ...
    assert "\x08" not in final_context or final_context.count("\x08") < 5, (
        "Too many backspace characters"
    )
    ...
    assert max_consecutive_blanks <= 3, (
